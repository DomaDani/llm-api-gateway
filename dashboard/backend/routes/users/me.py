...
from dashboard.backend.db import is_user_administrator, is_user_project_manager
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(_bearer), project_id: Optional[int] = None) -> UserDisplayInformation:
    ...
    user.is_admin = await is_user_administrator(user.id)
    if project_id is not None:
        user.is_project_manager = await is_user_project_manager(user.id, project_id)
